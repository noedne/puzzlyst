...
const Player = cc.Class.extend({
  ...
  showEntityTiles(opacity, fadeDuration, preview) {
    const scene = this.getScene();
    const gameLayer = scene && scene.getGameLayer();
    if (gameLayer && SDK.GameSession.current().getIsPlaying()) {
      let entityNode;
      let sdkEntity;
      let showTiles;
      const isForMyPlayer = this.getIsMyPlayer();
        ...
        if (this.getHoverDirty()) {
          ...
          if (gameSession.getIsEditing() && selectedSdkEntity !== null) {
            if (!mouseIsOnBoard) {
              ...
              gameSession.removeCardFromBoardWhileEditing(selectedSdkEntity);
            ...
            if (
              ...
              gameSession.moveEntity(selectedSdkEntity, mouseBoardPosition);
